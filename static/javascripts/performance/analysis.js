...
$(function (){
    ...
    function bing(year,quarter){
        //$("#sss").children("div").attr("data-percent",99);
        light.doget("/performance/getCompanyBonusRatioAll", {year:year, quarter:quarter}, function(err, result) {
            //console.log("result",result);
            var result = result;

            //for(var i=0;i<result.length;i++){
            //    var AWARD = result[i].AWARD;
            //    var SUM = parseInt(result[i].SUM);
            //    var NAME = result[i].NAME;
            //
            //    $(".percentage").data('easyPieChart').update(SUM);
            //
            //
            //    console.log("AWARD",AWARD,"SUM",SUM,"NAME",NAME);
            //}
            var i = 0;
            $('.percentage').each(function() {
                ...
                //$(this).
                $(this).data('easyPieChart').update(parseInt(result[i].SUM));
                $('span', this).text(result[i++].AWARD);
            });
            var o = 0;
            $('.label').each(function() {
               $(this).html(result[o++].NAME) ;
            });
    ...
    function setH(year, quarter){
        ...
        switch (quarter)
        {
            ...
            default:{alert("year choose error");return ;}break;
        }
        bing(year,quarter);

        light.doget("/performance/getCompanyBonusRatio", {year:year, quarter:quarter}, function(err, result) {
            var companyBonusRatio = result;
            // 路径配置
            require.config({
                paths:{
                    'echarts' : 'echarts',
         ...
         "sAjaxSource": "/performance/showJXFXTJ",     //指定要从哪个URL获取数据
         "fnServerData": function ( sSource, aoData, fnCallback, oSettings ) { //用于替换默认发到服务端的请求操作
         oSettings.jqXHR = $.ajax( {
         "dataType": "json",
         "type": "GET",
         "url": sSource,
         "data": {"year":year, "quarter":quarter,"query_type":1},
         "success": function(json) {
         if (json)
         {
         var table_data = json.data.data;
         fnCallback( {
         ...
         "columns": [
         {"title":"序号","mData": "N","width":"30px","sDefaultContent": ""},
         {"title":"部门","mData": "COMPANY_NAME","width":"30px","sDefaultContent": ""},
         {"title":"办事处","mData": "OFFICE_NAME","width":"30px","sDefaultContent": ""},
         {"title":"姓名","mData": "NAME","width":"30px","sDefaultContent": ""},
         { "title":"职位","mData": "POSITION","width":"30px" ,"sDefaultContent": ""},
         { "title":"奖金" ,"mData": "HS_AWARD","width":"30px","sDefaultContent": ""}
         ],
         "columnDefs": [
         {
         "targets": [4],
        ...
        $('#example2').dataTable( {
         ...
         "type": "GET",
         "url": sSource,
         "data": {"year":year, "quarter":quarter,"query_type":2},
         "success": function(json) {
         if (json)
         {
         var table_data = json.data.data;
         ...
         "type": "GET",
         "url": sSource,
         "data": {"year":year, "quarter":quarter,"query_type":3},
         "success": function(json) {
         if (json)
         {
         var table_data = json.data.data;
         ...
         "columns": [
         {"title":"序号","mData": "ROWNUM","width":"30px","sDefaultContent": ""},
         {"title":"区域","mData": "INDEXAREA_NAME","width":"30px","sDefaultContent": ""},
         {"title":"办事处","mData": "OFFICE_NAME","width":"30px","sDefaultContent": ""},
         {"title":"奖金","mData": "TRUE_TOTAL","width":"30px","sDefaultContent": ""},
         { "title":"同比","mData": "TB","width":"30px" ,"sDefaultContent": ""},
         { "title":"环比" ,"mData": "HB","width":"30px","sDefaultContent": ""}
         ...
         "type": "GET",
         "url": sSource,
         "data": {"year":year, "quarter":quarter,"query_type":4},
         "success": function(json) {
         if (json)
         {
         var table_data = json.data.data;
         ...
         "columns": [
         {"title":"序号","mData": "ROWNUM","width":"30px","sDefaultContent": ""},
         {"title":"区域","mData": "AREA_NAME","width":"30px","sDefaultContent": ""},
         {"title":"办事处","mData": "NAME","width":"30px","sDefaultContent": ""},
         {"title":"奖金","mData": "TRUE_TOTAL","width":"30px","sDefaultContent": ""},
         { "title":"同比","mData": "ntb","width":"30px" ,"sDefaultContent": ""},
         { "title":"环比" ,"mData": "nhb","width":"30px","sDefaultContent": ""}
        ...
        $(".showYear").html(year);
        $(".showQuarter").html(quarter);

        ////get from database;
        //light.doget("/performance/getPerAreaBonus",{"year":year, "quarter":quarter}, function (err, result){
        //    if (err || result===undefined || result.result_type===false)
        //    {
        //        alert("error");
        //    }
        //    else
        //    {
        //        var dat = result.result_data;
        //        if( $('#visit-chart, #demo-donut-chart').length > 0 ) {
        //            var data = new Array();
        //            var $ooo_div = $("#a0d8b").html("");
        //
        //            for (var i = 0; i < dat.length; i++)
        //            {
        //                data[i] = {label:dat[i].NAME,data:dat[i].AWARD};
        //                var $tem = $("<div class='col-md-3'> " +
        //                    "<div class='easy-pie-chart green' data-percent="+dat[i].AWARD/10000+"> " +
        //                    "<span class='percent'>"+dat[i].AWARD/10000+"</span> " +
        //                    "</div> " +
        //                    "<p class='text-center'>"+dat[i].NAME+"</p> " +
        //                    "</div>");
        //                $ooo_div.append($tem);
        //            }
        //
        ...
        //                    hoverable: true
        //                },
        //                colors: ["#d9d9d9", "#5399D6", "#d7ea2b", "#FF8C69"]
        //            });
        //        }
        //
        //
